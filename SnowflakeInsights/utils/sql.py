...
,ROUND(SUM(BYTES_SCANNED)/1000/1000/1000,2) AS GB_SCANNED
,ROUND(SUM(BYTES_SCANNED*PERCENTAGE_SCANNED_FROM_CACHE)/1000/1000/1000,2) AS GB_SCANNED_FROM_CACHE
,ROUND((SUM(BYTES_SCANNED*PERCENTAGE_SCANNED_FROM_CACHE) / SUM(BYTES_SCANNED))*100) AS PERCENT_SCANNED_FROM_CACHE
FROM "SNOWFLAKE"."ACCOUNT_USAGE"."QUERY_HISTORY"
WHERE START_TIME >= CONVERT_TIMEZONE('UTC', 'UTC', ('{date_from}T00:00:00Z')::TIMESTAMP_LTZ)
AND START_TIME < CONVERT_TIMEZONE('UTC', 'UTC', ('{date_to}T00:00:00Z')::TIMESTAMP_LTZ)
AND BYTES_SCANNED > 0
...
,round(avg(case when partitions_total > 0 then partitions_scanned / partitions_total else 0 end),2) avg_pct_scanned
from snowflake.account_usage.query_history
WHERE START_TIME >= CONVERT_TIMEZONE('UTC', 'UTC', ('{date_from}T00:00:00Z')::TIMESTAMP_LTZ)
AND START_TIME < CONVERT_TIMEZONE('UTC', 'UTC', ('{date_to}T00:00:00Z')::TIMESTAMP_LTZ)
group by 1, 2
...
,MIN(QUERY_ID)||' ; '||MAX(QUERY_ID) as REF_QUERY_IDS
FROM "SNOWFLAKE"."ACCOUNT_USAGE"."QUERY_HISTORY"
WHERE START_TIME >= CONVERT_TIMEZONE('UTC', 'UTC', ('{date_from}T00:00:00Z')::TIMESTAMP_LTZ)
AND START_TIME < CONVERT_TIMEZONE('UTC', 'UTC', ('{date_to}T00:00:00Z')::TIMESTAMP_LTZ)
...
QUERIES_BY_WAREHOUSE="""SELECT START_TIME,WAREHOUSE_SIZE,NUM_QUERIES,
ROUND(100 * RATIO_TO_REPORT(NUM_QUERIES) OVER (PARTITION BY START_TIME),2) AS PERCENT
FROM(
SELECT TO_DATE(START_TIME) AS START_TIME,
WAREHOUSE_SIZE,COUNT(*) AS NUM_QUERIES
FROM SNOWFLAKE.ACCOUNT_USAGE.QUERY_HISTORY
WHERE START_TIME >= '{date_from}'
AND START_TIME <= '{date_to}'
AND WAREHOUSE_SIZE IS NOT NULL
GROUP BY 1, 2 ORDER BY 1 DESC, 2)
ORDER BY START_TIME,WAREHOUSE_SIZE
"""
