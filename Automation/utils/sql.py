...
SUSPEND_TASK="ALTER TASK {arg2} SUSPEND"
GET_DDL="SELECT GET_DDL('TASK','{arg2}') as DDL"
...
STORED_PROC="""CALL  SP_JOB_SCRIPT('"""+DB_NAME+"."+SCHEMA_NAME+""".JOB_SCRIPTS{arg1}', 
'{arg2}', '"""+DB_NAME+"."+SCHEMA_NAME+""".JOB_SCRIPTS_AUDIT_TABLE{arg1}','{arg3}','{arg4}') 
"""

STORED_PROC_RUN_ID="""CALL  SP_JOB_SCRIPT('"""+DB_NAME+"."+SCHEMA_NAME+""".JOB_SCRIPTS{arg1}', 
'{arg2}',{arg3}, '"""+DB_NAME+"."+SCHEMA_NAME+""".JOB_SCRIPTS_AUDIT_TABLE{arg1}','{arg4}','{arg5}')
"""
...
'{arg2}',{arg3}, '"""+DB_NAME+"."+SCHEMA_NAME+""".JOB_SCRIPTS_AUDIT_TABLE{arg1}','{arg4}')"""

AUDIT_LOGS="""SELECT * FROM """+DB_NAME+"."+SCHEMA_NAME+"""."JOB_SCRIPTS_AUDIT_TABLE{arg1}"
...
DAG="""SELECT PARENT,F.VALUE::VARCHAR CHILD_FULL,substr(F.VALUE::VARCHAR,(regexp_instr(F.VALUE::VARCHAR,'\\\.',1,1)+1)) CHILD 
FROM (select SCHEMA_NAME||'.'||NAME PARENT,PREDECESSORS CHILD from table(SNDBX_DEMO_DB.information_schema.task_dependents(task_name => '{arg2}', recursive => TRUE)) )MAIN,TABLE(FLATTEN(MAIN.CHILD))F"""


GET_CHILD="""SELECT '{arg2}' PARENT,NULL CHILD_FULL,NULL CHILD UNION SELECT PARENT,F.VALUE::VARCHAR CHILD_FULL,substr(F.VALUE::VARCHAR,(regexp_instr(F.VALUE::VARCHAR,'\\\.',1,2)+1)) CHILD 
FROM (select SCHEMA_NAME||'.'||NAME PARENT,PREDECESSORS CHILD from table(SNDBX_DEMO_DB.information_schema.task_dependents(task_name => '{arg2}', recursive => TRUE)) )MAIN,TABLE(FLATTEN(MAIN.CHILD))F"""
...
PROMO_INSERT="""MERGE INTO """+DB_NAME+"""."""+SCHEMA_NAME+""".JOB_SCRIPTS{arg2} AS TGT
USING 
(SELECT SCRIPT_NAME,JOB_ID,SEQ_ID,REGEXP_REPLACE(SQL_COMMAND,'INT_STAGE{arg1}/','INT_STAGE{arg2}/') SQL_COMMAND,CONTINUE_ON_ERROR,IGNORE_SCRIPT FROM JOB_SCRIPTS{arg1} where script_name='{arg3}') AS SRC 
...
GET_STAGE_FILE_NAME="""SELECT SEQ_ID,SQL_COMMAND FROM JOB_SCRIPTS{arg1} where script_name='{arg2}' AND SQL_COMMAND LIKE '%@%'"""

PROMO_VALIDATE="""SELECT SEQ_ID FROM """+DB_NAME+"""."""+SCHEMA_NAME+""".JOB_SCRIPTS{arg1}
WHERE SCRIPT_NAME = '{arg2}'
EXCEPT
SELECT SEQ_ID FROM(
SELECT * FROM """+DB_NAME+"""."""+SCHEMA_NAME+""".JOB_SCRIPTS{arg1}
JOIN (SELECT * FROM """+DB_NAME+"""."""+SCHEMA_NAME+""".JOB_SCRIPTS_AUDIT_TABLE{arg1}
QUALIFY ROW_NUMBER() OVER(PARTITION BY SCRIPT_NAME,SEQ_ID ORDER BY START_TIME DESC)=1
AND SCRIPT_NAME='{arg2}'AND STATUS='SUCCESS')
USING(SCRIPT_NAME,SEQ_ID)
WHERE SCRIPT_NAME ='{arg2}'
AND LAST_UPDATED_TIMESTAMP < START_TIME)"""

# PROMO_VALIDATE="""SELECT SEQ_ID FROM "SNDBX_DEMO_DB"."DEMO_WORK_INTERIM"."JOB_SCRIPTS{arg1}"
# WHERE SCRIPT_NAME = '{arg2}'
# EXCEPT
# SELECT SEQ_ID FROM "SNDBX_DEMO_DB"."DEMO_WORK_INTERIM"."JOB_SCRIPTS_AUDIT_TABLE{arg1}"
# QUALIFY ROW_NUMBER() OVER(PARTITION BY SCRIPT_NAME,SEQ_ID ORDER BY START_TIME DESC)=1
# AND SCRIPT_NAME='{arg2}'AND STATUS='SUCCESS'ORDER BY SEQ_ID
# """
